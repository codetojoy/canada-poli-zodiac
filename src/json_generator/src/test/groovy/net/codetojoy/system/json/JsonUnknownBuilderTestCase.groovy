...
class JsonUnknownBuilderTestCase {
    ...
    void testBuildForUnknown_en() {
        ...
        assertEquals(1, onNode["children"].size())
        ...
        def onChild = onNode["children"][0]
        assertEquals(onChild["name"], "Mozart")
        assertEquals(onChild["party"], "Independent")
        assertEquals(onChild["size"], 1000)
    }

    @Test
    void testBuildForUnknown_fr() {
        def name = "Mozart"
        def zodiac = "unknown"
        def province = "Ontario"
        def riding = "Salzburg"
